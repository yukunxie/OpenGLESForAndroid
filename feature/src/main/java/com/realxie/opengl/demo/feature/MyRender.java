...
public class MyRender extends HandlerThread {
    private static final String TAG = "GLThread";
    private EGLConfig eglConfig = null;
    private EGLDisplay eglDisplay = EGL14.EGL_NO_DISPLAY;
    private EGLContext eglContext = EGL14.EGL_NO_CONTEXT;

    private int m_program = -1;
    ...
    public void render(Surface surface, int width, int height){
        ...
        // 基于顶点着色器与片元着色器创建程序
        if (m_program == -1) {
            m_program = createProgram(verticesShader, fragmentShader);
            // 获取着色器中的属性引用id(传入的字符串就是我们着色器脚本中的属性名)
            m_vPosition = GLES20.glGetAttribLocation(m_program, "vPosition");
            m_uColor = GLES20.glGetUniformLocation(m_program, "uColor");
        }
        ...
        GLES20.glVertexAttribPointer(m_vPosition, 2, GLES20.GL_FLOAT, false, 0, vertices);
        // 允许顶点位置数据数组
        GLES20.glEnableVertexAttribArray(m_vPosition);
        // 设置属性uColor(颜色 索引,R,G,B,A)
        GLES20.glUniform4f(m_uColor, 0.0f, 1.0f, 0.0f, 1.0f);
        // 绘制
